...
{
    public class CalibrationAddOrEditPresenter
        : BasePresenterForAddOrEdit<Calibration>
    {
        MeasuringInstrument _measuringInstrument;

        public CalibrationAddOrEditPresenter(
          IDbBaseViewForAddOrEdit<Calibration> view,
        ...
        protected override async void TransmissionDtatInView()
        {
            ...
            try
            {
                ...
                ((ICalibrationAddOrEditView)_view).PrintData(_data, message, calibrators.Result);
        ...
        protected override string ConfigeAddOrEdit()
        {
            ...
            if (_operation == EnumOperation.Edit)
            {
                ...
                //todo убрать if, он тут лишний.И тогда Избавиться от Предупреждения
                if (_data != null) objectСlone = (Calibration)_data.Clone();
            }
            else
            {
                message = "Ввод новых данных";
                _data = new Calibration(); //(T?)Activator.CreateInstance(typeof(T));
